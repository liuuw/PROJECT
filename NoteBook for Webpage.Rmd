...
par(mfrow=c(2,1))
plot(county,sk,type = 'l',col='blue',ylab='skewness')
plot(county,kt,type = 'l',col='red',ylab='kurtosis')
```
...
LinRegEPI = lm(Dropout_Rate ~ TotalFederalRevenuePerStudent + unemployment_rate + single_male_with_children, 
               data = dropout)

summary(LinRegEPI)
```
Residuals, the difference between what this model predicts Y is ( Yp ), and the actual value of Y, should have a bell-shaped distribution.
Multicollinearity, if the correlation among the X is causing trouble.
Heteroscedasticity, if the bivariate relationship between the Residuals and the Yp is not constant. Predictability of the model should be constant.

##Logistic Regression
```{r}

LogitEPI_a = glm(Dropout_Rate ~ TotalFederalRevenuePerStudent + unemployment_rate + Region, 
                  data = dropout 
                  )
